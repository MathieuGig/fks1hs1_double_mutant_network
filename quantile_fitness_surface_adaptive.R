...
quantile_fitness_surface_adaptive = function(DT,Nq,Nv,span,Q) {
  ...
  #define grid vector
  q = seq(min(c(quantile(DT[,s_1],probs=0,na.rm=T),quantile(DT[,s_2],probs=0,na.rm=T))),
          max(c(quantile(DT[,s_1],probs = 1,na.rm=T),quantile(DT[,s_2],probs = 1,na.rm=T)))+0.015,
          length.out=Nq)
  
  #Fq: data.table for surface values at each gridpoint
  Fq = data.table(s_1=rep(q,length(q)),s_2=rep(q,each=length(q)))
  ...
  subDT = DT[sample(nrow(DT),min(c(Nv,nrow(DT)))),.(s_1,s_2,fitness_norm)]
  ...
  List = list()
  List$F_median_fit = loess(F_median ~ s_1 + s_2,data = Fq,span=0.2)
  List$F_lower_fit = loess(F_lower ~ s_1 + s_2,data = Fq,span=0.2)
  List$F_upper_fit = loess(F_upper ~ s_1 + s_2,data = Fq,span=0.2)
